...
export default {
  ...
  created() {
    ...
    for (i = 1; i < this.conversation.length; i = i + 2) {
      ...
      this.fullText += "<table style=\"width:20%; margin-left: auto; margin-right: auto;\" cellpadding=\"0\" cellspacing=\"0\">"
      for (f = 0; f < this.word.length; f++) {
        if (this.flag == 0) {
          this.fullText += "<tr><h3><b>" + this.conversation[i].text + "</h3></b></tr>";
          this.fullText += "<tr><th>Word</th><th>Analysis</th></tr>"
        }
        this.fullText += "<tr><td>" + this.word[f].value + "</td><td>" + this.word[f].pos + "</td></tr>";
        this.flag = 1;
      }
      this.flag = 0;
      this.fullText += "</table>";
